...
# INC-010 — Canary Release Failure: api-gateway v4.1.0 Error Spike
...
1. **Breaking config format change** — v4.1.0 updated `routeConfig` schema but deployment did not include updated ConfigMap
2. **Incomplete pre-prod validation** — staging uses a simplified routing config that did not expose the null reference
3. **Missing null guard in new routing code** — `getUpstreams()` not guarded against null on old config format
4. **Feature flag not toggled** — new routing logic gated on flag `enable_v2_routing` which was not enabled, leaving code in partially initialized state
...
1. Halted canary rollout immediately via deployment pipeline
   ```bash
...
3. Verified all traffic back on stable v4.0.9 pods
   ```bash
   kubectl get pods -n gateway -l app=api-gateway
   # 3 stable pods Running, canary scaled to 0
   ```

4. Updated ConfigMap with v4.1.0-compatible schema and re-queued deployment for next window
...
**Action items:**
- [x] Rolled back canary, restored full stable traffic
- [ ] Add ConfigMap schema validation to deployment pipeline
- [ ] Update staging routing config to mirror prod structure
- [ ] Document ConfigMap schema changes as a required field in release notes template
